...
public class Project {
  ...
  public static final String DIRECT_DISTANCES_FILENAME =
      "src/cs526/termProject/direct_distance.txt";
  ...
  public static void main(String[] args) {
    ...
    // Map to hold the Node labels and their respective index toi be used on the nodeLocationMatrix
    Map<String, Integer> indexMap = directDistance.getIndexMapFromFile();
    ...
    ClosestNode closestNode = new ClosestNode(
        indexMap,
        nodeLocationMatrix,
        directDistance.getDistanceIndexMapFromFile());

    /**
     * Using Algorithm 1 to get to Z.
     */
    while (!"Z".equals(node)) {
      // add the next node to the sequence of all nodes
      sequenceOfAllNodes.add(node);
      node = closestNode.getClosestDirectNodeToZ(node);
    }
    sequenceOfAllNodes.add(node);

    System.out.println(sequenceOfAllNodes.toString());


//    System.out.printf("The adjacent node for %s closest to Z is: %s%n", node, closestDirectNodeToZ);
//    System.out.printf("Its distance from Z is: %d", distanceMap.get(closestDirectNodeToZ));
  }
}
